...
function main(): void {
  ...
  for (const entry of readdirSync(componentsDir, { withFileTypes: true })) {
    ...
    for (const violation of findDisallowedTransitions(readFileSync(file, "utf8"))) {
  ...
  if (violations.length > 0) {
    console.error(`Disallowed transition properties (only transform-equivalents may transition):
${violations.map((v) => `  - ${v}`).join("\n")}`);
    process.exit(1);
  }
  console.log(`check-transitionable-property: ${count} components respect the allow-list`);
}
