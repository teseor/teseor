...
function checkAllPackages(): ViolationDetail[] {
  ...
  for (const pkgPath of listPackageJsons()) {
    ...
    const { violations } = checkPackageExports(dirname(pkgPath), pkgJson);
    for (const v of violations) {
      out.push({
        file: `${v.pkg}/package.json`,
        message: `exports[${JSON.stringify(v.key)}] -> ${v.target} (path does not exist)`,
      });
    }
  }
  return out;
...
export const rule: WorkspaceCheck = {
  kind: "workspace",
  triggers: ["{packages,apps}/*/package.json"],
  run: checkAllPackages,
  hint:
