...
const ALLOWLIST_FILE = resolve(REPO_ROOT, "scripts", ".doc-path-allowlist.txt");
...
type Violation = { file: string; line: number; reference: string };
...
function main(): void {
  ...
  for (const file of files) {
    ...
    for (const { reference, line } of findDanglingPaths(source, file, allowlist)) {
      violations.push({ file: relative(REPO_ROOT, file), line, reference });
    }
  }
  if (violations.length > 0) {
    const lines = violations.map(
      (v) => `  - ${v.file}:${v.line}  \`${v.reference}\` does not resolve`,
    );
    process.stderr.write(`Backtick-wrapped paths that do not resolve:\n${lines.join("\n")}\n`);
    process.exit(1);
  }
  process.stdout.write(`check-doc-paths: ${files.length} markdown files clean\n`);
}
