...
Per-component entries are generated by `scripts/repo/sync-labels.ts` from `specs/*.yaml` on every spec change. CI gate ensures the generated portion of `labeler.yml` matches the current spec list (similar to `gen-drift`).
