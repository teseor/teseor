...
| **test-e2e** | Playwright against the harness (`apps/harness`) + the docs production build (`apps/docs` via `astro preview`) | Cross-framework DOM-parity assertion fails, a contract snapshot diverges from baseline, or a docs component page emits a `console.error` / `pageerror` / non-OK HTTP response |
...
## test-e2e (cross-framework DOM parity + docs production smoke)

Playwright boots two servers (harness + docs preview) and runs two test suites:
...
**Docs production smoke** (`tests/docs/console-errors.spec.ts`) against the docs site (`apps/docs/`, built via `astro build` and served via `astro preview`). For every `specs/*.yaml` (non-underscore-prefixed) the test visits the matching `/components/<name>/` route and fails on any `console.error`, uncaught `pageerror`, non-OK document response, subresource 4xx/5xx, or failed network request. The page list derives from the specs directory at test-collection time, so new components are covered automatically.
