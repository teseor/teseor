...
| **test-e2e** | Playwright against the harness (`apps/harness`), the docs production build (`apps/docs` via `astro preview`), and a parallel docs dev server (`astro dev`) | Cross-framework DOM-parity assertion fails, a contract snapshot diverges from baseline, or a docs component page emits a `console.error` / `pageerror` / non-OK HTTP response under either the production build *or* the dev server |
...
## test-e2e (cross-framework DOM parity + docs prod & dev smokes)

Playwright boots three servers (harness, docs preview, docs dev) and runs two test suites split across three projects:

**Contract tests** (`tests/contract/<name>.spec.ts`) against the harness app (`apps/harness/`). For every `spec.examples` entry the test:
...
Canonicalization sorts attributes alphabetically and strips whitespace-only text nodes (React and Vue handle template whitespace differently). The test file itself is autogenerated by `gen-tests`; see `architecture/codegen-pipeline.md` § "Cross-framework contract tests". The contract suite is wired to the `chromium` project, which targets the harness `baseURL`.

**Docs smoke** (`tests/docs/console-errors.spec.ts`) against the docs site (`apps/docs/`). For every `specs/*.yaml` (non-underscore-prefixed) the test visits the matching `/components/<name>/` route and fails on any `console.error`, uncaught `pageerror`, non-OK document response, subresource 4xx/5xx, or failed network request. The page list derives from the specs directory at test-collection time, so new components are covered automatically. The spec runs under two projects:

- **`docs-prod`** — against the built artifact (`astro build` + `astro preview`). Catches what ships.
- **`docs-dev`** — against `astro dev`. Catches dev-only hydration regressions where Vite's optimizer serves a different runtime to the browser (cf. #791 — production React DOM bundle leaking into the optimizer's direct-import path under workspace `./src/*` exports). The Astro dev toolbar is disabled under this server (`ASTRO_DISABLE_DEV_TOOLBAR=1`) so its transient `504 Outdated Optimize Dep` chunks don't false-positive the smoke.
