...
- **Not gating dismissable-layer wiring on a spec flag.** `popover.dismissable: boolean` would default to `true` almost everywhere; an always-on wire-up in `useOverlay` is a single mental model. Tooltip gains document-level Escape (regardless of focus location) — a quiet improvement over its per-trigger `keydown`, not a regression.
...
- **Tooltip behavior change (narrow).** Gains document-level Escape via dismissable-layer (regardless of focus). The outside-pointer notification arrives but Tooltip already closes on `mouseleave`, so it's effectively a no-op channel for Tooltip. No spec change for Tooltip; existing behavior tests stay green.
