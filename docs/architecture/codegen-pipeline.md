...
**Hand-authored code in wrapper packages.** Per [ADR-0012](../ADR/0012-wrapper-internal-seam.md), wrapper packages split their source between one generator-emitted runtime file and framework-conventional folders for hand-authored code:

| Path | Holds |
| --- | --- |
| `packages/react/src/_runtime.ts` (and the Vue parallel `packages/vue/src/_runtime.ts`) — generated | Mechanical helpers — responsive runtime, `useActiveBreakpoint`, `asElement` |
| `packages/react/src/hooks/useOverlay.ts` and `packages/vue/src/composables/useOverlay.ts` — hand-authored | Cross-cutting library hooks (`useOverlay`, future `useEscapeStack`, ...) |
| `packages/react/src/components/Slot.tsx` and `packages/vue/src/components/Slot.ts` — hand-authored | Cross-cutting library components (`Slot`) |
| `packages/react/src/behavior/<name>.tsx` (and parallels) — hand-authored | Per-component stateful behavior, imported by the matching generated styling shell when `behavior !== none` |

None of the hand-authored paths carry the `AUTOGENERATED` banner; `gen-drift` only regenerates `_runtime.ts` and the per-spec wrapper files, so paths outside that set are safe to edit by hand.
