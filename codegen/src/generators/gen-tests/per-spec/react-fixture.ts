...
import { renderReactFixtureBody } from "../_shared/fixture-bodies.ts";
import { hasSlotProps, quote } from "../_shared/printers.ts";

export function renderReactFixtureFile(spec: Spec): string {
  ...
  const slotHelper = hasSlotProps(spec)
    ...
    : "";
  const labelHelper = entries.includes("{LABEL}") ? `const LABEL = "Label";\n` : "";
